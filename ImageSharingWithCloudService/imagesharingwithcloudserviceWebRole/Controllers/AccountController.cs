...
using imagesharingwithcloudserviceWebRole.Filters;
...
namespace imagesharingwithcloudserviceWebRole.Models
{
    ...
    public class AccountController : BaseController
    {
        ...
        public ActionResult ImageViews(string date)
        {
            CheckAda();
            ...
            {
                IEnumerable<LogEntry> entries = LogContext.Select(queryDate);
                return View(entries);
            }
            else
            {
                ...
                IEnumerable<LogEntry> entries = LogContext.Select(targetTime);
                return View(entries);
            }
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                ...
                if (db.Users.Find(userId).Active == false)
                {
                    ModelState.AddModelError("", "The user name or password provided is incorrect.");
                    return View(model);
                }
                bool ADA = db.Users.Find(userId).ADA;
        ...
        public ActionResult Deactivate()
        {
            ...
            {
                model.Add(new SelectItemView(u.Id, u.Userid, !u.Active));
            }
            return View(model);
        }
        ...
        [Authorize(Roles = "Administrator")]
        [ValidateAntiForgeryToken]
        public ActionResult Deactivate(List<SelectItemView> model)
        {
            CheckAda();
            var db = new ImageSharingDB();
            foreach (var umod in model)
            {
                User user = db.Users.Find(umod.Id);
                umod.Name = user.Userid;
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (ImageSharingDB db = new ImageSharingDB())
                {
                    User user = db.Users.FirstOrDefault(u => u.Userid.ToLower() == model.UserName.ToLower());
                    // Check if user already exists
                    if (user == null)
                    {
                        // Insert name into the profile table
                        db.Users.Add(new User { Userid = model.UserName });
                        db.SaveChanges();
